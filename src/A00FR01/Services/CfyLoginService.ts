...
export class CfyLoginService {
    ...
    DoLogin(UserName:string, Password:string, Empresa?, Filial?, Background:boolean = true, ShowError:boolean = true) {
        ...
        pipe(
            ...
            catchError((Error: IHttpError) => {
                ...
                if (ShowError) {
                    let _Mensagem = Error.Mensagem || (Error.Status == 401 ? "CFYLOGINSERVICE_ERROR_USUARIO_NAO_AUTENTICADO" : "CFYLOGINSERVICE_ERROR_SERVIDOR_INDISPONIVEL");
    ...
    private setUser(User: any, Password:string) {
        ...
        this.Log.Log(User, 3);
        let Empresas;
        let _User: UserParameter = new UserParameter();
        ...
        if (_User.AccessNumber) {
            this.Log.Log(`Usuario possui uma empresa e filial: Setando Empresa 
               ${User.CompanyBranch.CompanyDescription} e Filial 
               ${User.CompanyBranch.BranchDescription}`, 1);
            ...
            if (this.AppParameter.AfterLogin) {
                ...
                this.AppParameter.AfterLogin(User);
            }
        }
        else {
